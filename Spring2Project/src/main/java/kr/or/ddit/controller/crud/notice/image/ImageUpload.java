package kr.or.ddit.controller.crud.notice.image;
...
public class ImageUpload {
	...
	// CKEditor본문 내용에 이미지 업로드 하기
	@RequestMapping(value="/imageUpload.do")
	public String imageUpload(
			MultipartHttpServletRequest multiFile,
			HttpServletRequest req, HttpServletResponse resp) throws Exception {
		
		// 기존에는 html형식의 데이터를 리턴하는 방식이었으나, 
		// CKEditor4 특정 버전 이후부터 json데이터를 구성해서 리턴하는 방식으로 변경됨
		...
		if(file != null && file.getSize() > 0 && StringUtils.isNotBlank(file.getName())) {
			...
			if(file.getContentType().toLowerCase().startsWith("image/")) {
				if(file.getSize() > limitSize) { // 업로드한 파일 사이즈가 최대 크기보다 클 경우
					/*
					 	{	
					 		"uploaded" : 0,
					 		"error" : [
				...
				}else { // 파일 사이즈가 정상 범위일 경우
					/*
						{
							"uploaded" : 1,
							"fileName" : XXXXXXX-XXXXXXXX.jpg",
							"url" : "/resources/img/XXXXXXX-XXXXXXXX.jpg" 
						}
					...
					try {
						...
						// 폴더 구조가 존재하지 않는 경우 생성하고 존재하는 경우에는 건너 뜀
