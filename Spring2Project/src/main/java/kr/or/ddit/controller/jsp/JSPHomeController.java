...
public class JSPHomeController {
		 ...
		 * 			- JSP가 제공하는 지시자에는 page지시자, taglib지시자, include지시자로 총 세가지가 있다.
		 * 
		 * 		1) page지시자
		 ...
		 * 		     속성                 |      설명
		 * 		───────────────────────────────────────────────────────
		 * 		contentType   | JSP가 생성할 문서의 MIME타입과 문자 코드를 지정한다.
		 * 		pageEncoding  | JSP페이지 자체의 문자 코드를 지정한다.
		 ...
		 * 		uri           | TLD파일이 위치한 URI 혹은 파일 경로를 지정한다.
		 * 		───────────────────────────────────────────────────────
		 ...
		 * 		     속성                 |      설명
		 * 		───────────────────────────────────────────────────────
		 * 		file          | 포함할 파일 경로를 지정한다.
		 * 		───────────────────────────────────────────────────────
		 * 
		 * 		2. 스크립틀릿
		 * 		- 스크립틀릿(Scriptlet)은 JSP페이지에서 자바 코드를 실행할 때 사용하는 코드의 블록이다.
		 * 		- 스크립틀릿 코드의 블록은 "<%"와 "%>"사이 , 그리고 "<%="와 "%>"사이에 자바 코드를 포함할 수 있다.
		 * 		- 현재는 스크립틀릿을 사용하기보다는 JSTL과 같은 커스텀 태그 라이브러리와 EL을 조합해서 JSP를 구현하는 방법을 많이 사용한다.
		 * 
		 * 		3. 커스텀 태그 라이브러리
		 * 		- 스크립트 요소가 많아지면 JSP코드가 복잡해진다.
		 * 		   이러한 문제를 해결하기 위한 방법 중 하나로 "커스텀 태그의 사용"이 있다.
		 * 	  	   커스텀 태그를 사용하면 뷰를 표시하기 위해 로직을 공통화하거나 표현하기 복잡한 로직을 캡슐화할 수 있어서 JSP의 구현 코드를 간결하게 만들 수 있다.
		 ...
		 * 			- Map요소를 조회하는 경우 "속성명.프로퍼티명" 또는 "속성명[프로퍼티명]"을 지정한다.
		 * 
		 * 			# 사용가능한 연산자
		 ...
		 * 				연산자        |   설명
		 * 			───────────────────────────────────────────────────────────────────────────
		 * 			   &&(and)   |  두 피연산자 모두 true이면 boolean true를 반환하고 그렇지 않으면 false
		 * 			   ||(or)    |  두 피연산자 중 하나 또는 모두 true이면 boolean true를 반환하고 그렇지 않으면 false
