...
public class TransactionController {
	 ...
	 *  		 > 하나의 트랜잭션 수행 시 타 트랜잭션의 작업단위가 간섭하지 못한다.
	 *  
	 *          - 영속성 (Durability)
	 ...
	 * 				클래스나 메서드에 대해 @Transactional어노테이션을 부여하여 트랜잭션을 적용할 수 있다. 
	 * 
	 * 		# 서비스 구현 객체에 트랜잭션 적용
	 * 			회원정보를 저장하다가 실패하거나 회원 권한 정보를 저장하다가 실패하면 회원 테이블과 회원 권한 테이블 모두 저장되지 않는다.
	 * 			회원정보와 회원 권한정보 모두 저장에 성공해야 비로소 회원 테이블과 회원 권한 테이블에 저장이 이루어진다. 
	 * 			삭제 수정도 마찬가지의 방식으로 이루어진다. 
	 * 
	 * 		# @Transactional의 예외처리 견해
	 * 
	 * 			@Transactional을 적용하고 중간에 에러를 발생시킨 후
	 * 			롤백된 상태를 확인하려고 했으나 롤백이 되지 않고 데이터베이스에 데이터가 등록된다.
	 * 			(분명 롤백이 된다고 많은 기술 블로거들이 명시하고 있지만 아니었다...)
	 * 			그 이유는 스프링 프레임워크에서 @Transactional은 기본적으로 Checked Exception에 대해서는
	 * 			롤백처리를 하지 않도록 설계되어 있다.
	 * 			기본적으로 스프링에서 트랜잭션 처리는 RuntimeException계열(Unchecked Exception)이라면
	 * 			롤백처리를 한다.
	 * 
	 * 			여기서 트랜잭션으로 국한된 롤백 정책이 아니라 스프링 프레임워크에서의 기본 정책에 대한 내용일 뿐이다.
	 * 			즉 기본적인 스프링 트랜잭션의 정책이 있고, 
	 * 			그 안에서 정책 이외의 옵션으로 특정 에러가 발생했을 때 롤백을 진행할 수 있도록 제공한다.
	 * 
	 ...
	 *				
	 *				1) isolation (격리수준)
	 *					- 각 트랜잭션이 존재할 때 트랜잭션들끼리 서로 고립된 수준을 나타내며 서로 간에 가용된 데이터를
	 *					    컨트롤 할지에 대한 부분들을 설정할 수 있다.
	 *					- isolation기본값은 default이다.
	 *					- 새롭게 시작된 트랜잭션에만 적용되므로 Propagation.REQUIRED 또는 
	 *                    Propagation.REQUIRES_NEW와 함께 사용되도록 독점 설계 되었다.
	 *                  
	 *                  1-1) 옵션
	 *                  	
	 *                  	*** 용어  
	 ...
	 *                  	# Phantom Read
	 *                  		- 자신이 처리중인 트랜잭션에서 처리했던 내용 안에서 다른 트랜잭션이 데이터를 수정 후 
	 *                  	  	  Commit하더라도 자신의 트랜잭션에서 처리한 내용만 사용하는 것을 의미
	 *                  	
	 *                  - DEFAULT : 기본 데이터 저장소의 기본 격리 수준을 사용한다.
	 *                  	> 우리가 사용하는 Oracle은 READ_COMMITED를 사용하며,
	 *                  	  MySQL은 REPEATABLE_READ를 기본 격리 수준으로 사용한다.
	 *                  - READ_COMMITED : Dirty read가 방지됨을 나타내는 상수이다.
	 *                  		Non-repeatable Read 및 Phantom Read가 발생할 수 있다.
	 *                  	> 하나의 트랜잭션 처리가 이루어진 변경 내용이 Commit된 후  다른 트랜잭션에서 조회가 가능하다. 
	 *                  	> A트랜잭션이 데이터를 변경하고 커밋을 하지 않은 상태에서 B트랜잭션이 조회를 진행할 때
	 *                  	  B트랜잭션은 Shared lock이 걸린다.
	 *                  	> 트랜잭션 A에서 memId의 값을 'a001'에서 'a002'로 변경 후 커밋을 진행하지 않고
	 *                  	    트랜잭션 B에서 memId를 조회하면 'a001'이 조회된다.
	 *                  	    하지만 트랜잭션 A에서 커밋을 진행 후 트랜잭션 B에서 조회를 해보면 'a002가 조회된다'.
	 *                  - READ_UNCOMMITED : Dirty read, Non-repeatable Read 및 Phantom ream가 발생할 수 있음을 나타내는 상수
	 *                  	> 다른 트랜잭션의 내용이 Commit 또는 Rollback되거나 되지 않아도 다른 트랜잭션에서 조회가 가능
	 *                  	> 트랜잭션  A에서 memId의 값을 'a001'에서 'a002'로 벼경 후 커밋하지 않더라도 트랜잭션 B에서 
	 *                  		memId를 조회해보면 'a002'로 노출되는 걸 확인할 수 있다.
	 ...
	 *                  	> 트랜잭션 Commit이 일어난 데이터에 대해서 조회가 가능(트랜잭션 완료시까지 조회에 대한 
	 *                  		Shared lock이 걸리진 않음)
	 *                  	> 트랜잭션 A에서 memId를 조회해본 결과 'a001'이고, 트랜잭션 B에서 memId를 'a002'로 변경 후
	 *                  		커밋을 진행한다. 이때 트랜잭션 A에서는 여전히 조회결과가 'a001'로 나타난다.
	 *                  		그리고 트랜잭션 A에서 커밋 후 재조회를 해보면 변경된 'a002'가 조회된다.
	 *                  		트랜잭션 A는 조회된 결과가 하나의 스냅샷에 저장되어 있고, 저장되어 있는 스냅샷 정보를 꺼내다보니 
	 *                  		'a001'로 표시된다. 커밋 후 기존 스냅샷에서 커밋 된 결과가 스냅샷으로 저장되기 때문에
	 *                          변경된 'a002'를 데이터로 확인할 수 있다.
	 *                          
	 *                - SERIALIZABLE : Dirty read, Non-repeatable read 및 Rhantom Read가 방지됨을
    *                            나타내는 상수입니다.
    *                   > Phantom Read가 발생하지 않는다.
    *                   > 현재의 트랜잭션 처리가 커밋되지 않으면 다른 트랜잭션이 실행되지 않는다.
    *                   > 트랜잭션 A에서 memId 'a002'로 변경 후 커밋합니다. 이때, 다른 트랜잭션은 트랜잭션 A가
    ...
    *                     > 클래스라인에 선언적 트랜잭션 어노테이션이 걸려있을때, 특정 메소드만 트랜잭션을 처리하지 않겠다고
    *                        했을때 사용할 수 있는 속성
    *            
    *            		3) readOnly (읽기전용)
